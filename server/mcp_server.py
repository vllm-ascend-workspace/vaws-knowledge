...
* every response carries `service_api_version`, `layers_available`,
  `layers_absent` (with reasons) and `degraded`;
...
A caller that only understands an older `service_api_version` can therefore
still tell the difference between "no such fact" and "could not answer".
"""
...
def handle_message(service: KnowledgeService, message: Mapping[str, Any]) -> dict[str, Any] | None:
    ...
    if method == "initialize":
        return None if is_notification else _result(
            request_id,
            {
                ...
                # Top level as well: a caller probing capability should not
                # have to know where we hid the version.
                "service_api_version": SERVICE_API_VERSION,
    ...
    if method == "ping":
        return None if is_notification else _result(request_id, {"service_api_version": SERVICE_API_VERSION})
    ...
    if method == "tools/list":
        return None if is_notification else _result(
            request_id, {"tools": TOOLS, "service_api_version": SERVICE_API_VERSION}
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python3 -m server.mcp_server",
        description="stdio MCP server over the vaws-knowledge three-layer corpus",
    )
    ...
    try:
        service = KnowledgeService(config_path=args.config)
