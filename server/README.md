...
Runtime dependency: PyYAML, and even that is soft — see
[requirements.txt](requirements.txt). Nothing here imports `torch`,
`torch_npu`, or touches NPU hardware; the service reads and writes text
documents.
...
python3 -m server.mcp_server --describe

# stdio MCP server
python3 -m server.mcp_server [--config path/to/vaws-knowledge.json]
```

## Tools

All three tool payloads carry the same envelope: `service_api_version`,
`layers_available`, `layers_absent` (layer → reason), `degraded`,
`absent_fact_semantics: "unknown"` and `degradation_contract`.
