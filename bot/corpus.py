...
DEPENDENCY_HINT = (
    "bot: PyYAML is not installed. Create a venv and install the bot "
    "requirements:\n"
...
def repo_root(start: Path | None = None) -> Path:
    """Locate the repository root: the nearest ancestor holding ``schemas/``.
    ...
    for candidate in (here, *here.parents):
        if (candidate / "schemas" / "knowledge-v2.schema.json").is_file():
            return candidate
