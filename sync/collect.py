...
PRIVATE_SOURCE_RECIPE = (
    ...
    "that source, run this repository's `python3 tools/export.py "
...
def _run_export(
    ...
    *,
    ...
    script = tools_dir / "export.py"
    if not script.is_file():
        return GateResult("export.py", "unavailable", "gate unavailable: tools/export.py is missing")
    cmd = [
        python,
        str(script),
        str(source),
...
def run_collection(
    *,
    ...
    tools_dir = pathlib.Path(tools_dir).resolve()
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    tools_dir = pathlib.Path(args.tools_dir).resolve() if args.tools_dir else repo / "tools"
