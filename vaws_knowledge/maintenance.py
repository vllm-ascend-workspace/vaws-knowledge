...
def project_config(project: Path, path: Path | None = None) -> ServiceConfig:
    ...
    experience_layers = payload.setdefault("experience", {}).setdefault("layers", {})
    experience_layers.setdefault("project", {"roots": [str(project / ".agents" / "experiences")]})
    experience_layers.setdefault("candidate", {"roots": [str(project / ".vaws-local" / "experience" / "candidate")]})
