...
SCHEMA = PACKAGE / "schemas" / "knowledge-v2.schema.json"


def _checkout_root() -> pathlib.Path | None:
    here = pathlib.Path.cwd().resolve()
    for candidate in (here, *here.parents):
        if (candidate / "examples" / "valid-entry.yaml").is_file():
            return candidate
    return None


_CHECKOUT = _checkout_root()
REPO = _CHECKOUT or PACKAGE
EXAMPLE = (REPO / "examples" / "valid-entry.yaml") if _CHECKOUT else pathlib.Path("/nonexistent/examples/valid-entry.yaml")

from vaws_knowledge.conformance import reference
