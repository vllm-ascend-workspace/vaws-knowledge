...
2. ``schema``      — ``vaws-knowledge validate <paths>``     (external CLI)
3. ``redaction``   — ``vaws-knowledge redact --check <paths>`` (external CLI)
4. ``canonical``   — covered by validate (content_hash)
...
External tools are invoked as CLI contracts and never imported. They run as
``python -m vaws_knowledge <subcommand>`` after the package is installed.
"""
...
from vaws_knowledge.bot import conflicts as _conflicts
from vaws_knowledge.bot import dedup as _dedup
from vaws_knowledge.bot import integrity as _integrity
from vaws_knowledge.bot import staleness as _staleness
from vaws_knowledge.bot.corpus import DependencyError, LoadResult, load_paths, relpath, repo_root
...
EXTERNAL_GATES: tuple[tuple[str, str, str, tuple[str, ...]], ...] = (
    # id, title, subcommand, fixed args
    ("schema", "Schema conformance and content_hash", "validate", ()),
    ("redaction", "Redaction re-scan", "redact", ("--check",)),
)
...
def run_external_gate(
    ...
    *,
    ...
    argv = [python, "-m", "vaws_knowledge", command, *fixed_args, *paths]
    label = " ".join(["python3", "-m", "vaws_knowledge", command, *fixed_args, *paths])
    try:
        ...
        return GateResult(
            ...
            ERROR,
            True,
            f"gate error → fail closed: `{label}` exceeded {timeout}s",
        )
        ...
        return GateResult(
            ...
            ERROR,
            True,
            f"gate error → fail closed: could not execute `{label}`: {exc.__class__.__name__}",
        )
    ...
    if proc.returncode == 0:
        return GateResult(gate_id, title, PASS, True, f"`{label}` exited 0", lines)
    return GateResult(
        gate_id,
        title,
        FAIL,
...
def run_gates(
    ...
    *,
    ...
    for gate_id, title, command, fixed in EXTERNAL_GATES:
        if mode == "audit" and gate_id == "schema":
            # The audit re-scans what already landed; schema is a PR-time gate.
            continue
        results.append(run_external_gate(gate_id, title, command, fixed, rel_paths, root, python=python))
