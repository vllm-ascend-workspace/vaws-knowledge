...
def _dependency_message(module: str) -> str:
    return (
        ...
        "  Install the tooling dependencies into a virtualenv:\n"
        "    python3 -m venv .venv\n"
        "    .venv/bin/pip install -r requirements.txt\n"
        "  then run this tool with .venv/bin/python3."
    )
...
def bootstrap_package() -> None:
    """Allow ``python3 tools/x.py`` to import siblings as ``tools.x``.

    When a file inside ``tools/`` is executed directly, ``sys.path[0]`` is the
    ``tools/`` directory itself, so ``from tools import canonical`` fails. Put
    the repository root on the path instead.
    """
    root = str(REPO_ROOT)
